...
class LearnerWorker:
    ...
    def update_world_model(self,history,Obs,Obs_state, Action1,Reward,gpu_buffer,batch_size, experience_size,timing):
        ...
        for i in range(self.n):  # n=1, only ally model their oppo, n=2, both of ally and oppo model their oppo
            kl_loss = 0
            if i==0:
                action=Action1[1].repeat(1,self.cfg.num_adversaries,1) # T,N*k,4
                Action0=action.reshape(Action1[1].shape[0],Action1[1].shape[1],-1) # T,N,4K
                Action=torch.cat((Action1[0],Action0),dim=-1) # T,N,4*(k+1), k: num_oppo_obs
                # action0 = Action1[1].reshape(Action1[1].shape[0], -1)
                # action = action0.unsqueeze(1).repeat(1,self.cfg.num_adversaries,1) # T,K*4->T,N,k4
                # Action0 = action.reshape(Action1[1].shape[0], Action1[1].shape[1], -1)  # T,N,4K
                # Action = torch.cat((Action1[0], Action0), dim=-1)  # T,N,4*(k+1), k: num_oppo_obs
            else:
                action=Action1[0].repeat(1,self.cfg.num_good_agents,1) # T,N*k,4, all_obs_oppo(joint)
                Action0=action.reshape(Action1[0].shape[0],Action1[0].shape[1],-1)
                Action=torch.cat((Action1[1],Action0),dim=-1)
