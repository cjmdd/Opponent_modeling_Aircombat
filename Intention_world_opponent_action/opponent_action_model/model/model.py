...
class RecurrentStateSpaceModel(nn.Module):
    def __init__(self, cfg, action_space, state_dim, action_dim, rnn_hidden_dim, num_agents, num_neighbor_obs,
        ...
        self.hyper_embedding_mlp_w = Hypernet(cfg, input_dim=state_dim + self.action_dim*(1+num_oppo_obs), hidden_dim=hidden_dim,
                                              main_input_dim=state_dim + self.action_dim*(1+num_oppo_obs), main_output_dim=state_dim,
                                              )
    ...
    def posterior_intention_latent(self, next_obs, history):
        ...
        intention_latent=torch.cat((q_intention,q_latent),dim=-1)
        action_pre=self.linear(intention_latent) # T,N*num_obs_oppo, action_space

        return q_intention, q_latent,action_pre
    ...
    def rollout(self, steps, obs_model, reward_model, rl_policy, obs0, history, delta_s, state_post, lambda_t, idx):
        ...
        with torch.no_grad():
            ...
            action = rl_policy.forward2(obs1.reshape(-1, obs1.shape[-1]), idx)[0].reshape(obs1.shape[0], -1,
                                                                                        self.action_space.shape[0])
            if idx==0:
                action_oppo=action[:, self.num_agents:].repeat(1,self.num_agents,1) #T,N/2,D
                action=torch.cat((action[:,:self.num_agents],action_oppo.reshape(obs.shape[0],obs.shape[1],-1)),dim=-1)
            else:
                action_oppo = action[:, :-self.num_agents].repeat(1, self.num_agents, 1)  # T,N/2,D
                action = torch.cat((action[:, -self.num_agents:], action_oppo.reshape(obs.shape[0], obs.shape[1], -1)),
                                   dim=-1)
            ...
            for t in range(steps):
                ...
                # state_post2 = state_post.transpose(1, 0)
                # state_post2 = state_post2.reshape(N, -1)
                # obs_t2 = obs_all2.transpose(1, 0)
