...
class TransformerLatentLayer(TransformerIntentionLayer):
    ...
    def forward20(self,neighbor_obs,k_latent,q_latent,q_intention):
        ...
        Gating_outputs = self.gating(neighbor_obs, self_attention)
        ...
        Gating_intention = self.gating_intention(q_intention.transpose(1, 2).reshape(T, -1, D),
                                                 self_intention_attention)
        Position_input = self.layer_norm_intention(Gating_intention)
        Gating_y2 = self.positionforward(Position_input)
        transformer_out_intention = self.gating2(Gating_intention, Gating_y2)
        ...
        Gating_cross = self.gating_cross(q_intention_cross, cross_latent_attention)
    ...
    def forward3(self,neighbor_obs,k_latent,q_latent,q_intention):
        ...
        Gating_intention = self.gating_intention(q_intention.transpose(1,2).reshape(T,-1,D), self_intention_attention)
        Position_input = self.layer_norm_intention(Gating_intention)
        Gating_y2 = self.positionforward(Position_input)
        transformer_out_intention = self.gating2(Gating_intention, Gating_y2)
