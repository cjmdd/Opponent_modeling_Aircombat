...
from sample_factory.model.model import HyperJD2TStateSpaceModel, ObsModel, RewardModel
...
class LearnerWorker:
    ...
    def update_world_model(self,history,Obs,Obs_state, Action1,Reward,gpu_buffer,batch_size, experience_size,timing):
        ...
        next_state_posteriors = []
        delta_s_posteriors = []
        Loss_delta_s_prior_multi = []
        Reward_loss = []
        Obs_loss = []
        for i in range(self.n):  # n=1, only ally model their oppo, n=2, both of ally and oppo model their oppo
            ...
            intent_latent = torch.cat((intention, latent_strategy), dim=-1).reshape(Obs[i].shape[0],
                                                                                          Obs[i].shape[1],
                                                                                          -1)  # T,N,K*2D
            obs_intention = torch.cat((Obs[i], intent_latent), dim=-1)
            Action=self.actor_critic.forward2(obs_intention.reshape(-1, obs_intention.shape[-1]),i)[0].reshape(Obs[i].shape[0],Obs[i].shape[1],-1) # Joint scheme
            ...
            loss_model = loss_delta_s_prior_multi + obs_loss + reward_loss
            ...
            log.debug(
                'update_step: %3d loss: %.5f, loss_delta_s_prior_multi: %.5f, obs_loss: %.5f, reward_loss: % .5f'
                % (self.num_rollouts[0],
                   loss_model.item(),
            ...
            fake_buffer = self.rssm[i].rollout_policy(gpu_buffer, self.cfg.horizon, self.obs_model[i],
                                                      self.reward_model[i],
                                                      self.actor_critic, Obs_state[i], history[i],
        ...
        return loss_rollout_ac,Obs_loss,Reward_loss,Loss_delta_s_prior_multi
    ...
    def _train(self, gpu_buffer, batch_size, experience_size, timing):
        ...
        if not self.cfg.intention_model:
            loss_rollout_ac,obs_loss,reward_loss,loss_delta_s_prior_multi=self.update_world_model(history_obs, Obs,Obs_state, Action, Reward, gpu_buffer, batch_size, experience_size, timing)



            # stats_and_summaries = self._MEGCTRxppo('real', gpu_buffer,pu_buffer.values.shape[0],
            #                                        gpu_buffer.values.shape[0], Loss_delta_s_prior_multi, Obs_loss,
            #                                        Reward_loss, i, timing)
